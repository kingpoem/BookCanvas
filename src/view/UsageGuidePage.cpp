...
UsageGuidePage::UsageGuidePage(QWidget* parent)
    ...
    addSection(
        ...
        tr("跨时域"),
        tr("请先在「全局配置」将 <code>topology</code> 设为 "
           "<code>chiplet_mesh</code>，再在画布上对 "
           "路由器/终端做芯粒分组（可用内置 mesh 拓扑块铺网）。导出时会合并芯粒坐标与 "
           "<code>chiplet_die_*</code> 等字段。"
           "不同芯粒内的路由器之间用普通「点两点连边」即可，连线呈偏红色；其它跨芯粒边可为青色装饰"
           "，"
           "右键可编辑全局 D2D/CDC 参数。"),
        tr("<h4>画布操作</h4>"
           "<ul>"
           "<li>仅当「全局配置」中 <code>topology</code> 为 <b>chiplet_mesh</b> "
           "时可创建芯粒分组；从该页返回 Canvas "
           "或点击「创建芯粒」时，会把全局页当前选项同步到各网络 "
           "Tab 的场景。画布上仍可用内置 mesh 拓扑块生成路由器网格。</li>"
           "<li>右键芯粒组框（含标题区域）：可打开「芯粒仿真参数」，设置网格坐标 "
           "(cx,cy)、片内 k、片内延迟与 die 时钟（对应 BookSim <code>chiplet_die_*</code> "
           "数组项）。</li>"
           "<li>连接两个不同芯粒内路由器的边为<b>偏红色</b>样式（与普通连边操作相同）；"
           "涉及终端等其它跨芯粒边可为青色短划光晕与中点标记。"
           "右键跨芯粒边可选择「跨芯粒 D2D / CDC 全局参数」——对应 "
           "<code>chiplet_d2d_latency</code>、<code>chiplet_cdc_*</code>、"
           "<code>chiplet_intra_latency</code>、<code>chiplet_connect</code> "
           "等（仿真为全局配置）。</li>"
           "</ul>"
           "<h4>导出合并规则（摘要）</h4>"
           "<ul>"
           "<li>存在芯粒分组且全局已为 <code>chiplet_mesh</code> 时，导出会写入 "
           "<code>chiplet_x</code> / <code>chiplet_die_*</code> 等合并字段，并保持 "
           "<code>topology=chiplet_mesh</code>；请同时保证 "
           "<code>routing_function=dim_order_chiplet_mesh</code>。</li>"
           "<li><code>chiplet_x</code> / <code>chiplet_y</code> 由所有芯粒坐标最大值推得；"
           "按行优先（cy 外层、cx 内层）生成 <code>chiplet_die_k</code>、"
           "<code>chiplet_die_intra_latency</code>、<code>chiplet_die_clock_period</code>、"
           "<code>chiplet_die_clock_phase</code> 的括号列表字符串；空网格格点用当前 mesh 的 k "
           "与默认时钟填"
           "充。</li>"
           "<li><code>chiplet_connect</code> 取自画布侧「跨芯粒」全局对话框或 "
           "<code>bookcanvas_chiplet_mesh.connect</code>（x或 xy）。</li>"
           "</ul>"
           "<h4>主要配置键说明</h4>"
           "<ul>"
           "<li><code>chiplet_x</code> / <code>chiplet_y</code>：芯粒网格在 X/Y 方向的 die "
           "个数。</li>"
           "<li><code>chiplet_k</code>：各 die 均匀 k 时的默认值；有异构 k 时以 "
           "<code>chiplet_die_k</code> 列表为准。</li>"
           "<li><code>chiplet_connect</code>：die 间 D2D 是否在 X 向、或 X+Y 向同时存在（x / "
           "xy）。</li>"
           "<li><code>chiplet_intra_latency</code>：片内链路基准延迟；"
           "<code>chiplet_die_intra_latency</code> 可按 die 覆盖，0 表示回退到前者。</li>"
           "<li><code>chiplet_d2d_latency</code>：跨芯粒物理线延迟（与 CDC 模块配合）。</li>"
           "<li><code>chiplet_cdc_enable</code> 及 <code>chiplet_cdc_*</code>：异步跨时钟域时的 "
           "FIFO 深度、"
           "同步周期等；异周期 die 时钟需开启 CDC。</li>"
           "<li><code>chiplet_die_clock_period</code> / <code>chiplet_die_clock_phase</code>："
           "各 die 相对最细仿真 tick 的整数时钟与相位。</li>"
           "</ul>"
           "<p>键名与约束以「文件路径」一节所列 BookSim 文档为准。</p>"),
        QStringLiteral("chiplet mesh芯粒 跨时域 chiplet_mesh dim_order_chiplet_mesh D2D CDC "
                       "bookcanvas_chiplets 跨芯粒"));
