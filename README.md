...
- **BookSim2 / `booksim` 可执行文件**：由本仓库子模块 `3rdpart/booksim2` **在本地用 Makefile 编译**，不再从网络下载预编译包。需要与 **GNU Make** 兼容的 `make`（Windows 上一般为 **MSYS2** 提供的 `mingw32-make`）、**flex**、**bison** 以及 **MinGW-w64 g++**（与 MSYS2 UCRT64 工具链一致）。`booksim` 与主程序 **BookCanvas** 使用不同工具链是正常的：主程序用 MSVC，booksim 用子模块自带 Makefile + MinGW。
